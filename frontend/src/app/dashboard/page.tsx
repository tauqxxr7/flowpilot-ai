      ...
      <Panel title="Recent activity">
        <div className="space-y-3">
          {stats.recent_activity.length ? stats.recent_activity.map((item) => (
            <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-line bg-panel p-3">
              <div>
                <p className="font-mono text-sm font-semibold">{item.id}</p>
                <p className="text-sm text-gray-600">{item.intent}</p>
              </div>
      ...
      {Object.entries(data).map(([label, value]) => (
        <div key={label}>
          <div className="mb-1 flex justify-between text-sm">
            ...
            <div className="h-2 rounded-full bg-accent" style={{ width: `${Math.max(8, (value / total) * 100)}%` }} />
          </div>
        </div>
      ))}
    </div>
